...
export async function PUT(request: Request) {
  ...
  if (!originCheck.ok) {
    return json({
      ok: false,
      code: "ORIGIN_DENIED",
      message: "La solicitud de edición no proviene de esta aplicación.",
  ...
  if (
    ...
    return json({
      ok: false,
      code: "VALIDATION_FAILED",
      message: "La solicitud de edición no es válida.",
  ...
  if (typeof product.id !== "string" || !productIdPattern.test(product.id)) {
    return json({
      ok: false,
      code: "VALIDATION_FAILED",
      message: "Selecciona un producto existente para editarlo.",
  ...
  try {
    ...
    return json({
      ...
      message: `No fue posible actualizar el producto. La información permanece en el formulario. Referencia: ${requestId}.`,
      correlationId: requestId,
    }, 500);
  }
}
