...
export function AccountsPayableManager({
  ...
  const initialPurchaseNumber = payables.find((payable) => payable.purchase_id === initialPurchaseId)?.purchase_number ?? "";
  const [query, setQuery] = useState(initialPurchaseNumber);
  const [supplierFilter, setSupplierFilter] = useState("all");
  const [statusFilter, setStatusFilter] = useState(initialPurchaseId ? "all" : "open");
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,420px)]">
          ...
          <div className="min-w-0 space-y-4">
            ...
            <FormPanel title="Registrar pago a proveedor">
              ...
              <Button type="button" onClick={() => runAction(registerSupplierPaymentAction(paymentDraft), () => setPaymentDraft(emptyPaymentDraft()))} disabled={!canManage} pending={isPending} pendingLabel="Registrando pago…"><ReceiptText size={16} />Registrar pago</Button>
            </FormPanel>
            ) : null}
