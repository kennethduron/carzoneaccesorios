...
export function PosWorkspace({ operatorName }: { operatorName: string }) {
  ...
  const markItems = useCallback((next: PosDraftItem[]) => { setItems(next); dirtyRef.current = true; setIsDirty(true); pendingSaveKeyRef.current = null; setStatus(navigator.onLine ? "dirty" : "offline"); setMessage(""); }, []);
  const markDelivery = useCallback((next: PosDeliveryState) => { setDelivery(next); dirtyRef.current = true; setIsDirty(true); pendingSaveKeyRef.current = null; setStatus(navigator.onLine ? "dirty" : "offline"); setMessage(""); }, []);
  ...
  const saveDraft = useCallback(async () => {
    ...
    try {
      ...
      dirtyRef.current = false; pendingSaveKeyRef.current = null; applyDraft(saved, saved.validationStatus === "warning" ? "Guardado con advertencias que deben revisarse antes del cierre." : "Borrador guardado."); await loadActiveDrafts();
    } catch (error) {
