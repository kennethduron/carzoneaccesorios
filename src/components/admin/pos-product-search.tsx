...
function productAvailability(product: PosProductSearchResult) {
  ...
  if (product.autoDisabledByStock || availableStock <= 0) return { label: "No disponible", className: "text-red-700", available: false };
  if (availableStock <= product.lowStockThreshold) return { label: `Stock bajo · Disponible: ${availableStock}`, className: "text-amber-700", available: true };
  return { label: `Disponible: ${availableStock}`, className: "text-emerald-700", available: true };
}
...
export function PosProductSearch({ disabled, customerId, customerCommercialVersion, onAdd }: Props) {
  ...
  return <section className="min-w-0 rounded-xl border border-black/10 bg-white p-3 shadow-sm" aria-labelledby="pos-product-search-title">
    <div className="flex items-center justify-between gap-3"><h2 id="pos-product-search-title" className="font-semibold">Productos</h2><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">Precios actualizados</span></div>
    <div className="relative mt-2"><Search aria-hidden="true" className="pointer-events-none absolute left-3 top-3 text-black/40" size={18} /><input ref={inputRef} autoFocus disabled={disabled} role="combobox" aria-autocomplete="list" aria-expanded={results.length > 0} aria-controls={listboxId} aria-activedescendant={activeIndex >= 0 ? `${listboxId}-option-${activeIndex}` : undefined} aria-describedby="pos-product-search-help" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={keyDown} placeholder={disabled ? "Selecciona el cliente del borrador" : "Buscar y agregar productos"} className="min-h-11 w-full rounded-lg border border-black/15 pl-10 pr-10 text-sm outline-none focus:border-[#e4252c] focus:ring-2 focus:ring-[#e4252c]/15 disabled:bg-slate-100" />{loading ? <LoaderCircle aria-label="Buscando productos" className="absolute right-3 top-3 animate-spin motion-reduce:animate-none text-[#e4252c]" size={18} /> : null}</div>
    <p id="pos-product-search-help" className="mt-1 text-xs text-black/50">Mínimo 2 caracteres. Flechas, Enter o Escape.</p>
