...
export function AccountsReceivableManager({ data, canMarkPaid, canExport }: { data: AdminAccountsReceivablePage; canMarkPaid: boolean; canExport: boolean }) {
  ...
  const closeAction = () => { if (!isPending) { setActionMode(null); setDraft(null); } };
  const submitAction = () => {
        ...
        <div className="min-w-0 rounded-xl border border-black/10 bg-white shadow-sm">
          ...
          <div role="table" aria-label="Cuentas por cobrar" className="min-w-0">
            ...
            {data.rows.length ? data.rows.map((row) => {
              ...
              return <article role="row" key={row.id} className={`relative grid min-h-[150px] gap-2 border-b border-black/5 p-3 last:border-0 lg:min-h-0 lg:grid-cols-[1.05fr_.9fr_1fr_.75fr_.8fr_.75fr_.85fr_.65fr] lg:items-center ${active ? "bg-red-50/80 before:absolute before:inset-y-0 before:left-0 before:w-1 before:bg-[#e30613]" : ""}`}>
                <div role="cell" className="min-w-0"><div className="flex items-center justify-between gap-2 lg:block"><strong className="truncate">{row.customer_name}</strong><span className={`rounded-full px-2 py-1 text-xs lg:hidden ${statusClass(row.status)}`}>{statusLabels[row.status]}</span></div></div>
...
function ReceivableDetail({ row, canCollect, onClose, onPayment, onMarkPaid }: { row:AdminAccountsReceivableRow; canCollect:boolean; onClose:()=>void; onPayment:()=>void; onMarkPaid:()=>void }) {
  return <aside aria-label={`Detalle de ${row.customer_name}`} className="h-fit rounded-xl border border-black/10 bg-white p-4 shadow-sm"><div className="mb-3 flex items-start justify-between"><div><p className="text-sm font-semibold">Detalle de cuenta</p><h3 className="text-lg font-bold">{row.customer_name}</h3></div><button onClick={onClose} className="grid size-11 place-items-center rounded-lg" aria-label="Cerrar detalle"><X size={18}/></button></div><ReceivableDetailContent row={row}/><div className="mt-4"><DetailActions canCollect={canCollect} onPayment={onPayment} onMarkPaid={onMarkPaid}/></div></aside>;
}
...
function DetailActions({canCollect,onPayment,onMarkPaid}:{canCollect:boolean;onPayment:()=>void;onMarkPaid:()=>void}) { return <div className="grid grid-cols-2 gap-2"><Button onClick={onPayment} disabled={!canCollect} className="min-h-11"><PlusCircle size={17}/> Registrar abono</Button><Button variant="ghost" onClick={onMarkPaid} disabled={!canCollect} className="min-h-11"><CheckCircle2 size={17}/> Marcar como pagado</Button></div>; }
