alter table public.accounts_receivable
  ...
  add column if not exists payment_received_reference text,
  add column if not exists payment_recorded_by uuid references public.users(id) on delete set null;

alter table public.accounts_receivable
  drop constraint if exists accounts_receivable_payment_received_method_valid;

alter table public.accounts_receivable
  add constraint accounts_receivable_payment_received_method_valid
  check (
    payment_received_method is null
    or payment_received_method in ('bank_transfer', 'card', 'cash')
  );
...
alter table public.accounts_receivable
  drop constraint if exists accounts_receivable_payment_reference_length;

alter table public.accounts_receivable
  add constraint accounts_receivable_payment_reference_length
  check (payment_received_reference is null or char_length(payment_received_reference) <= 200);

insert into public.notification_preferences (notification_type, module, label, internal_enabled, email_enabled, destination_roles)
values ('commercial_credit.enabled', 'pagos', 'Crédito comercial habilitado', true, false, array[]::text[])
...
drop policy if exists "Customers can read own internal notifications" on public.internal_notifications;
create policy "Customers can read own internal notifications" on public.internal_notifications
  for select
  using (user_id = auth.uid());

drop policy if exists "Customers can update own internal notifications" on public.internal_notifications;
create policy "Customers can update own internal notifications" on public.internal_notifications
  for update
  using (user_id = auth.uid())
