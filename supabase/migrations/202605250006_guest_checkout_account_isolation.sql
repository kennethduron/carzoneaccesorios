-- Keep guest checkout isolated from registered accounts.
-- A guest order must never be linked to an existing account only because the
-- checkout email, phone or RTN matches.

revoke execute on function public.create_checkout_order_legacy_20260511(
  text,
  text,
  text,
  text,
...
begin
  for order_row in
    select
      orders.id,
      orders.customer_id as old_customer_id,
      ...
      orders.created_at
    from public.orders
    join public.customers linked_customer on linked_customer.id = orders.customer_id
    where orders.user_id is null
      and linked_customer.user_id is not null
  loop
    select customers.id
    into guest_customer_id
    from public.customers
    where customers.user_id is null
      and order_row.email is not null
      and lower(customers.email) = lower(order_row.email)
      and public.normalize_hn_phone(customers.phone) = public.normalize_hn_phone(coalesce(order_row.phone, order_row.customer_phone))
    ...
    if guest_customer_id is null then
      ...
      values (
        null,
        coalesce(nullif(trim(order_row.customer_name), ''), 'Cliente invitado'),
        lower(nullif(trim(coalesce(order_row.email, '')), '')),
        coalesce(public.normalize_hn_phone(coalesce(order_row.phone, order_row.customer_phone)), 'Sin telefono'),
        null,
        nullif(trim(coalesce(order_row.delivery_address, '')), ''),
        nullif(trim(coalesce(order_row.delivery_city, '')), ''),
        true,
        'active',
        'cliente',
        '[CHECKOUT_INVITADO] Cliente separado automaticamente de una cuenta registrada'
      )
    ...
    set customer_id = guest_customer_id,
        updated_at = now()
    where payments.order_id = order_row.id;

    update public.invoices
    set customer_id = guest_customer_id,
        updated_at = now()
    where invoices.order_id = order_row.id;

    update public.crm_followups
    set customer_id = guest_customer_id,
        updated_at = now()
    where crm_followups.order_id = order_row.id;

    update public.crm_notes
    set customer_id = guest_customer_id
    where crm_notes.order_id = order_row.id;
    ...
    values (
      'orders',
      order_row.id,
      'guest_checkout_account_isolation',
      jsonb_build_object('customer_id', order_row.old_customer_id),
      jsonb_build_object('customer_id', guest_customer_id, 'reason', 'guest order detached from registered customer')
    );
  end loop;
end;
$$;
