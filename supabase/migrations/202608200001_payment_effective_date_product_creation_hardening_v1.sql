-- Supplier-payment effective-date support and atomic product-create hardening.
-- No business rows are backfilled or rewritten by this migration.
-- All existing recognition, reversal, period, balance and payment guards remain authoritative.

create or replace function public.resolve_accounts_payable_payment_recognition_v2(
...
begin
  ...
  if direct_count = 1 then
    ...
    reason_value := case
      ...
      when p_effective_payment_date is null
...
revoke all on function
  public.resolve_accounts_payable_payment_recognition_v2(uuid, date, uuid)
...
comment on function
  public.resolve_accounts_payable_payment_recognition_v2(uuid, date, uuid)
is
  'Supplier-payment recognition resolver. Preserves canonical publication, reversal, ambiguity, opening-balance and duplicate-payment guards while allowing an authoritative effective payment date before a later recognition journal.';

-- Every current v1 call site is supplier-payment-specific (eligibility,
-- registration, assessment, outbox processing or reversal). Keep the public
-- signature as a compatibility delegate so the atomic database workflows and
-- the application preflight use one rule.
...
set search_path = public, pg_temp
as $$
  select public.resolve_accounts_payable_payment_recognition_v2(
    p_accounts_payable_id,
    p_proposed_journal_date,
    p_payment_id
  );
$$;
...
comment on function
  public.resolve_accounts_payable_accounting_recognition_v1(uuid, date, uuid)
is
  'Compatibility delegate for supplier-payment recognition. Use resolve_accounts_payable_payment_recognition_v2 for new callers.';

-- Catalog save, image replacement and an authorized initial stock adjustment
-- now share one database transaction. A failure in any write rolls back all of it.
create or replace function public.save_product_catalog_v3_locked(
...
declare
  saved record;
  stock_movement_value uuid := null;
  stock_before_value integer;
  stock_after_value integer;
  stock_quantity_value integer := 0;
begin
  select * into strict saved
  from public.save_product_catalog_v2_locked(
    target_product_id,
    product_data,
    images_data
  );
  ...
  if target_stock is not null then
    select
      stock_row.movement_id,
      stock_row.stock_before,
      stock_row.stock_after,
      stock_row.quantity
    into strict
    ...
    from public.set_product_stock_locked(
      saved.product_id,
      target_stock,
      'Guardado atomico desde catalogo de productos'
    ) as stock_row;
  else
    select stock into strict stock_before_value
  ...
  stock_movement_id := stock_movement_value;
  stock_before := stock_before_value;
...
revoke all on function
  public.save_product_catalog_v3_locked(uuid, jsonb, jsonb, integer)
  from public, anon;
grant execute on function
  public.save_product_catalog_v3_locked(uuid, jsonb, jsonb, integer)
...
comment on function
  public.save_product_catalog_v3_locked(uuid, jsonb, jsonb, integer)
is
  'Atomic product catalog, images and optional authorized stock initialization wrapper over the existing locked V2 contracts.';
...
begin
  ...
  then
    raise exception using
      errcode = '55000',
      message = 'El pago multifÃ¡ctura todavÃ­a no estÃ¡ habilitado.';
  end if;
  ...
  if exists (
    ...
    raise exception using
      errcode = '23514',
      message = 'La compra no corresponde al proveedor de la obligaciÃ³n.';
  end if;
  ...
  accounting_occurred_at := effective_at;
  ...
  -- A published canonical recognition remains eligible when its journal
  -- date is later than the accounting user's authoritative payment date.
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'effective_payment_precedes_recognition_date', exists (
        select 1 from pg_temp.supplier_multi_payment_input_v1 input
        where input.recognition_date > accounting_date
      ),
      'backdated_recognition_count', (
        select count(*) from pg_temp.supplier_multi_payment_input_v1 input
        where input.recognition_date > accounting_date
      ),
      'request_key_suffix', right(p_request_key::text, 8),
      'result', 'started'
...
comment on function public.register_supplier_multi_payment_v1(
  ...
  'Atomic HNL supplier payment with 1-200 applications, authoritative effective payment date, SHA-256 idempotency, deterministic locks and one V2 outbox.';
-- Routing and worker validation retain the recognition fingerprint and all
-- canonical evidence checks, but no longer reject solely because a published
-- recognition journal is later than the authoritative payment date.
...
begin
  ...
  set scenario = 'multi_application_v1',
      metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
        ...
        'effective_payment_precedes_recognition_date', recognition_max_date > journal_date,
