-- Account-based wholesale access. Codes remain archived for historical order
-- references, but no longer grant or validate current wholesale pricing.

alter table public.customers
  add column if not exists wholesale_status text not null default 'none'
  check (wholesale_status in ('none', 'pending', 'approved', 'rejected', 'suspended'));

update public.customers
set wholesale_status = case
  when is_wholesale = true and active = true and status = 'active' then 'approved'
  when is_wholesale = true and (active = false or status = 'disabled') then 'suspended'
  when status = 'pending_account' or coalesce(notes, '') like '%[SOLICITUD_MAYOREO]%' then 'pending'
  when is_wholesale = true and status = 'inactive' then 'rejected'
  else wholesale_status
end
where wholesale_status = 'none';

create index if not exists customers_wholesale_status_account_idx
  on public.customers(wholesale_status, user_id, active);
...
as $$
  select
    upper(trim(coalesce(raw_code, ''))) as code,
    false as is_valid,
    'disabled'::text as status,
    'El acceso mayorista ahora depende del estado aprobado de la cuenta.'::text as message,
    false as requires_login,
...
as $$
  select
    ...
    null::timestamptz,
    0::integer,
    null::public.wholesale_code_status
  where false;
$$;

revoke execute on function public.validate_wholesale_code_public(text) from anon, authenticated;
revoke execute on function public.activate_wholesale_account(text) from authenticated;
