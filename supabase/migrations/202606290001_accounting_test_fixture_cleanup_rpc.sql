-- Phase 2C: restricted cleanup path for controlled accounting test fixtures.
-- This intentionally does not restore table-level delete grants or broad delete policies.
...
declare
  ...
  service_role_call boolean := current_setting('request.jwt.claim.role', true) = 'service_role';
...
begin
  ...
  if not service_role_call and (actor_user_id is null or not public.has_permission('accounting:manage')) then
    raise exception 'Solo usuarios autorizados pueden limpiar fixtures contables de prueba.';
  end if;
