-- Keep the current fiscal flow intact while allowing a commercial-credit
-- invoice to be issued independently from payment confirmation.
...
comment on function public.generate_fiscal_invoice_from_order(uuid) is
  'Generates fiscal invoices. Commercial credit invoices are independent from payment confirmation. CAI deadline uses Honduras time.';
