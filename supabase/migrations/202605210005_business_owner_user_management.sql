...
create policy "Authorized staff can manage users"
...
create or replace function public.has_operational_owner()
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    ...
    where users.active = true
      and roles.name in ('admin', 'business_owner')
  );
$$;

create or replace function public.count_active_operational_owners()
returns integer
language sql
stable
security definer
...
begin
  ...
  if actor_record.role_name = 'business_owner' then
    if target_record.role_name not in ('cliente', 'vendedor', 'bodega', 'contadora')
       or normalized_target_role not in ('cliente', 'vendedor', 'bodega', 'contadora') then
...
begin
  ...
  if actor_record.role_name = 'business_owner'
     and target_record.role_name not in ('cliente', 'vendedor', 'bodega', 'contadora') then
