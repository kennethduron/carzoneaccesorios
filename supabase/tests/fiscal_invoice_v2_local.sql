...
declare
  ...
  fiscal_request_id uuid := '95200000-0000-4000-8000-000000000030';
  recovery_request_id uuid := '95200000-0000-4000-8000-000000000031';
  replay_generated record;
  recovered_generated record;
begin
  ...
  from public.generate_fiscal_invoice_from_order_v2(fiscal_request_id, order_id);
  select * into replay_generated
  from public.generate_fiscal_invoice_from_order_v2(fiscal_request_id, order_id);
  select * into recovered_generated
  from public.generate_fiscal_invoice_from_order_v2(recovery_request_id, order_id);
  if generated.replayed
    or not replay_generated.replayed
    or replay_generated.invoice_id is distinct from generated.invoice_id
    or recovered_generated.invoice_id is distinct from generated.invoice_id then
    raise exception 'FISCAL_V2_IDEMPOTENCY_MISMATCH: first=%, replay=%, recovered=%',
      to_jsonb(generated), to_jsonb(replay_generated), to_jsonb(recovered_generated);
  end if;
