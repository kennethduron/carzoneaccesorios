...
try {
  ...
  const { data: wholesaleCustomer, error: wholesaleError } = await admin.from("customers").update({ is_wholesale: true, wholesale_status: "approved", wholesale_approved_at: new Date().toISOString(), active: true, user_id: owner.userId, company_name: `${marker} Empresa` }).eq("id", customerId).select("commercial_version").single(); assert.ifError(wholesaleError);
